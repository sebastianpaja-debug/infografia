# Infografía (HTML + CSS + JS)

Este proyecto quedó listo para desplegarse como sitio **estático** usando solo:

- `index.html`
- `styles.css`
- `script.js`

Ubicación: `public/infografia/`.

## Cómo subirlo en tu hosting (arroba)

1. Entra al administrador de archivos de tu hosting.
2. Abre la carpeta pública del dominio (normalmente `public_html`).
3. Sube **únicamente** estos 3 archivos desde `public/infografia/`:
   - `index.html`
   - `styles.css`
   - `script.js`
4. Verifica que queden en la **misma carpeta**.
5. Abre tu dominio y limpia caché si no ves cambios.

## Importante

- No necesitas Node.js, npm, ni backend.
- Si tu hosting bloquea Google Fonts, la página seguirá funcionando con fuentes de respaldo.


- No hay dependencias de `secciones/*.html`; todo el contenido está embebido en `index.html`.
